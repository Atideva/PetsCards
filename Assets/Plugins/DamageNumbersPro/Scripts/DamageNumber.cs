...
namespace DamageNumbersPro
{
    ...
    {
        public event Action<DamageNumber> OnCombination = delegate { };
        public event Action<DamageNumber> OnDead = delegate { };
        ...
        void HandleCombination()
        {
            ...
            if (myAbsorber != null)
            {
                ...
                //Move:
                position = Vector3.Lerp(position, myAbsorber.position,
                    Time.deltaTime * combinationSettings.targetSpeed);

                //Scale:
                baseScale = Vector3.Lerp(baseScale, Vector3.zero,
                    Time.deltaTime * combinationSettings.targetScaleDownSpeed);

                //Fading:
                absorbProgress += Time.deltaTime;
                float normalizedProgress = absorbProgress / Mathf.Max(0.001f, combinationSettings.absorbTime);

                baseAlpha = Mathf.Min(baseAlpha,
                    Mathf.Max(0, 1 - normalizedProgress * combinationSettings.targetFadeOutSpeed));
                UpdateAlpha(currentFade);

                if (normalizedProgress >= 1)
                {
                    myAbsorber.OnCombination(myAbsorber);
                    GiveNumber();
                    DestroyNumber();
                }
            }
            ...
            {
                //Move:
                position = Vector3.Lerp(position, myTarget.position,
                    Time.deltaTime * combinationSettings.absorberSpeed);
            }
        }
        ...
        void ApplyTransform()
        {
            ...
            if (enableShaking)
            {
                ...
                if (IsAlive())
                {
                    ...
                    {
                        finalPosition = Vector3.Lerp(ApplyShake(finalPosition, fadeInShake), idleShakePosition,
                            currentFade);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        finalPosition = Vector3.Lerp(ApplyShake(finalPosition, fadeOutShake), idleShakePosition,
                            currentFade);
                    }
                    else
                    {
            ...
            if (enableCombination)
            {
                currentScale = Vector3.Lerp(currentScale, baseScale,
            ...
            if (enablePerspective)
            {
                ...
                if (targetCamera == null && Camera.main != null &&
                    (Camera.main.orthographic == false || ignoreOrthographic == false))
                {
                ...
                if (targetCamera != null)
                {
                    ...
                    if (consistentScale)
                    {
                        ...
                        {
                            appliedScale *= perspectiveSettings.farScale +
                                            (perspectiveSettings.closeScale - perspectiveSettings.farScale) *
                                            Mathf.Clamp01(1 - (distance - perspectiveSettings.closeScale) /
                                                Mathf.Max(0.01f,
                                                    perspectiveSettings.farDistance - perspectiveSettings.closeScale));
                        }
                    }
        ...
        void TryCombination()
        {
            ...
            if (combinationSettings.absorberType == AbsorberType.OLDEST)
            {
                ...
                {
                    if (otherNumber != this && otherNumber.myAbsorber == null &&
        ...
        public void GiveNumber()
        {
            if (!givenNumber)
            {
                ...
                myAbsorber.currentScale = new Vector3(myAbsorber.baseScale.x * combinationSettings.absorberScaleGain.x,
                    myAbsorber.baseScale.y * combinationSettings.absorberScaleGain.y, 1);
            }
        }
        ...
        public void InitVariables()
        {
            ...
            if (enableMovement)
            {
                if (moveType == MoveType.LERP)
                {
                    remainingOffset = new Vector2(Random.Range(lerpSettings.minX, lerpSettings.maxX),
                        Random.Range(lerpSettings.minY, lerpSettings.maxY));
                }
                else
                {
                    currentVelocity = new Vector2(Random.Range(velocitySettings.minX, velocitySettings.maxX),
                        Random.Range(velocitySettings.minY, velocitySettings.maxY));
                }
            }
        ...
        public void SetFade(float progress, FadeSettings fadeSettings)
        {
            ...
            scaleA.z = 1;
            Vector3 scaleB = Vector2.Lerp(new Vector3(1f / scaleOffset.x, 1f / scaleOffset.y, 1) * fadeSettings.scale,
                Vector2.one, progress);
            scaleB.z = 1;
        ...
        public void RemoveFromCombination()
        {
            ...
            {
                if (combinationDictionary != null &&
                    combinationDictionary.ContainsKey(combinationSettings.combinationGroup))
                {
